...
- (instancetype)initWithRequestHandle:(DDMIRequestHandle *)requestHandle{
    self = [super initWithNibName:@"DDMILoginViewController" bundle:DDMISDKBundle];
    if (self) {
...
- (void)viewDidLoad {
    ...
    NSString * bundlePath = [[ NSBundle mainBundle] pathForResource: @"DDMIResource"ofType :@ "bundle"];
    NSString *imgPath= [bundlePath stringByAppendingPathComponent:@"dd_input_bg_verification@2x.png"];
    UIImage *image_1=[UIImage imageWithContentsOfFile:imgPath];
    
    UIImage *image = [UIImage imageNamed:@"dd_input_bg_verification@2x.png" inBundle:DDMISDKBundle compatibleWithTraitCollection:nil];

    self.inputbackImageView.image = DDMISDKBundleImage(@"dd_input_bg_verification@2x.png");
